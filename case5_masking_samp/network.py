...
class Dial2vec(nn.Module):
    ...
    def forward(self, data):
        ...
        """
        if len(data) == 8:
            input_ids, attention_mask, token_type_ids, role_ids, turn_ids, position_ids, stage_ids, labels = data
        else:
            input_ids, attention_mask, token_type_ids, role_ids, turn_ids, position_ids, stage_ids, labels, guids = data
        ...
        stage_ids = stage_ids.view(stage_ids.size()[0] * stage_ids.size()[1], stage_ids.size()[-1])
        ...
        stage1_mask = torch.where(stage_ids == 1, one_mask, zero_mask)
        stage2_mask = torch.where(stage_ids == 2, one_mask, zero_mask)
        stage3_mask = torch.where(stage_ids == 3, one_mask, zero_mask)
        stage4_mask = torch.where(stage_ids == 4, one_mask, zero_mask)
        # print(stage1_mask.shape)  # torch.Size([110, 512])
        # print(stage2_mask[0,:])
        
        stage1_attn_mask = (attention_mask * stage1_mask)
        stage2_attn_mask = (attention_mask * stage2_mask)
        stage3_attn_mask = (attention_mask * stage3_mask)
        stage4_attn_mask = (attention_mask * stage4_mask)
        # print(stage2_attn_mask[0])  # torch.Size([110, 512])
        # print(stage1_attn_mask.unsqueeze(-1).shape)
        # print("==================self_output")
        # print(self_output[0, :, :])
        # print(stage1_attn_mask.unsqueeze(-1)[0, :, :])
        
        stage1_self_output = self_output * stage1_attn_mask.unsqueeze(-1)  # torch.Size([110, 512, 1])
        stage2_self_output = self_output * stage2_attn_mask.unsqueeze(-1)
        stage3_self_output = self_output * stage3_attn_mask.unsqueeze(-1)
        stage4_self_output = self_output * stage4_attn_mask.unsqueeze(-1)
        ...
        w2 = torch.matmul(stage2_self_output, (stage1_self_output.transpose(-1, -2)+stage3_self_output.transpose(-1, -2)+stage4_self_output.transpose(-1, -2)))
        ...
        if turn_ids is not None:
            ...
            filtered_w1 = w1 * view_range_mask
        ...
        for i in range(0, self.sample_nums):
            if i==1:
                # print(stage_pooled_output[:, i, :])
                continue
            # print(output_embeddings[:, 0, :].shape)
            cos_output = self.calc_cos(pooled_output_embeddings[:, i, :], stage_pooled_output[:, i, :])   #torch.Size([10, 11, 768])
            # print(cos_output)
