...
class DialogueFeatures():
    def __init__(self, input_ids, input_mask, segment_ids, role_ids, label_id, domain_id, turn_ids=None, position_ids=None, guid=None):
...
class DialogueDataset():
    ...
    def load_data(self):
        ...
        features = [self.get_dialfeature(row) for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing")]
    ...
    def get_dialfeature(self, example):  # 기존: data2tensor(self, line, type), pos, neg 각각 실행
        """
        ...
        if self.type=='train':    
            ...
            domain_label = example['label']

        else:
            sample_list = [example['conversation'].split(sample_sep_token)[0]]  # positive sample 1개, negative sample 9개
            role_list = [int(r) for r in example['turn'].split(sample_sep_token)] \
                        if example['turn'].find(turn_sep_token) != -1 \
                        else [int(r) for r in example['turn']]
            domain_label = example['label']
...
def get_loader(args, metric):
    ...
    if args.train == 'True' and args.test == 'False':
        ...
        loader = {'train': train_iter._get_loader(train_features),
                  'valid': valid_iter._get_loader(valid_features)}
