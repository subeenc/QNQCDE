...
class Dial2vec(nn.Module):
    ...
    def repeat_tensor_elements(self, multi_ls):
        repeated_rows = multi_ls.repeat_interleave(10, dim=0)
    ...
    def forward(self, data, strategy='mean_by_role', output_attention=False): 
        ...
        """
        if len(data) == 9:
            input_ids, attention_mask, token_type_ids, multi_ids, role_ids, turn_ids, position_ids, labels, multi_ls = data
        else:
            input_ids, attention_mask, token_type_ids, multi_ids, role_ids, turn_ids, position_ids, labels, multi_ls, guids = data
        ...
        multi_ids = multi_ids.view(multi_ids.size()[0] * multi_ids.size()[1], multi_ids.size()[-1])
        ...
        all_multi_ls = self.repeat_tensor_elements(multi_ls)
        
        # one_mask = torch.ones_like(multi_ids)
        # zero_mask = torch.zeros_like(multi_ids)
        # role_a_mask = torch.where(multi_ids == 0, one_mask, zero_mask)
        # role_b_mask = torch.where(multi_ids == 1, one_mask, zero_mask)
        
        role_a_mask = (all_multi_ls[:, 0].unsqueeze(1) == multi_ids).float()
        role_b_mask = (all_multi_ls[:, 1].unsqueeze(1) == multi_ids).float()
