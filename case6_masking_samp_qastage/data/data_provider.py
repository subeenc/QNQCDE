...
class DataProvider():
    ...
    def __convert_examples_worker_for_bert(self, worker_index, start_index, end_index, examples):
        ...
        """
        features = []
        self.logger.debug("converting_examples, worker_index: %s start: %s end: %s" % (worker_index, start_index, end_index)) # 주의: qa 추가 안함
        ...
        # for data_index, example in enumerate(examples):
            ...
            for t, (s, qa) in enumerate(zip(sample_list, qa_list)): # 수정: qa 추가, for문을 돌며 하나의 대화에 대한 qa 턴이 들어오는 것
                ...
                if not t:
                    ...
                    pos_input_ids, pos_turn_ids, pos_role_ids, pos_segment_ids, pos_position_ids, pos_input_mask, pos_qa_ids = self.create_positive_sample(context, role_list, eou, qa)
                    ...
                    sample_qa_ids.append(pos_qa_ids) # 수정: qa 추가
    ...
    def create_positive_sample(self, context, role_list, eou, qa):
        text_tokens = []
        text_turn_ids = []
        text_role_ids = []
        text_segment_ids = []
        ...
        return text_input_ids, text_turn_ids, text_role_ids, text_segment_ids, text_position_ids, text_input_mask, text_qa_ids
