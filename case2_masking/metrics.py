...
def get_rankings(scores_from_model, dtype='float64'):
    ...
    :return:
    """
    rank1 = np.argsort(-scores_from_model.astype(dtype), axis=-1) # scores_from_model 배열에 대해 내림차순으로 정렬된 순위(rank)를 계산, 기준은 코사인 유사도, 내림차순으로 정렬
...
def mean_reciprocal_rank(scores_from_subject, scores_from_model, dtype='float64'):
    ...
    # print("scores_from_model", scores_from_model.shape, scores_from_model)
    # print("scores_from_subject", scores_from_subject.shape, scores_from_subject)
    # print("sorted_scores_from_subject", sorted_scores_from_subject.shape, sorted_scores_from_subject)
...
def feature_based_evaluation_at_once(features, labels, gpu_features=None, n_average=1,
    ...
    if 'clustering' in tasks:
        ...
        for _ in range(n_average): # n_average 횟수만큼 반복하여 평균을 계산
            ...
            # print("=====clsutering_features=====") 
            # print(features.shape) # (661, 768)
            clf.fit(features)
