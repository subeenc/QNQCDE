...
class WrapperBert:
    ...
    def eval_tasks(
        ...
        # is_best 업데이트 방식 변경: purity, SR, MAP 중 2개 이상의 성능이 향상되었을 경우, is_best=True 할당
                    ...
                    if global_step % self.args.test_interval == 0:
                        ...
                        if is_best: # dev 기준
                            ...
                            is_best_test, test_evaluation_result = self.eval_tasks(strategy='mean_by_role',
                                                                        tasks=['clustering', 'semantic_relatedness', 'session_retrieval', 'align_uniform'],
                                                                        mode='test')
                            if is_best_test: # test 성능도 향상되면 저장하도록 수정
                                ...
                                if self.args.local_rank in [-1, 0]:
                                    ...
                                    self.logger.info('***** is_best_dev -> [%s], is_best_test -> [%s] | Save best model to -> [%s] on LOCAL_RANK=%s' % (is_best, is_best_test, self.best_model_name, self.args.local_rank))
